...
use ashfall_rendering::beauty_v22::{
    BeautyCellPackageV22, BeautyMaterialIdV22, BeautySurfaceIdV22, Bounds3V22, CurbSegmentV22,
    FacadeModuleV22, GroundedWaterFilmV22, HumanPoseStateV22, HumanProxyV22, LandfillPropKindV22,
    LandfillPropV22, NaturalEnvironmentV22, PlantInstanceV22, RoadPatchV22, StoneInstanceV22,
    SurfaceTextureRecipeV22, Vec3V22, VehicleKindV22, VehicleProxyV22, WorldBiomeV22,
    sample_material_v22,
};
...
pub struct BeautyV22GoldenCaptureReport {
    pub image_paths: Vec<PathBuf>,
    pub timing_path: PathBuf,
    pub timings: Vec<BeautyV22CaptureTiming>,
    pub scene_build_ms: f32,
}

#[derive(Clone, Debug, PartialEq)]
pub struct BeautyV22CaptureTiming {
...
pub fn capture_beauty_v22_golden_scenes(
    output_dir: impl AsRef<Path>,
) -> io::Result<BeautyV22GoldenCaptureReport> {
    ...
    let captures = [
        (WorldBiomeV22::City, "ashfall_v22_city.bmp"),
        (WorldBiomeV22::NatureReserve, "ashfall_v22_nature.bmp"),
        (WorldBiomeV22::Landfill, "ashfall_v22_landfill.bmp"),
    ];
    ...
    for (biome, file_name) in captures {
        ...
        timings.push(BeautyV22CaptureTiming {
            biome,
            render_ms,
            write_ms,
    ...
    let timing_path = output_dir.join("ashfall_v22_frame_timings.txt");
    write_timing_report(
        &timing_path,
        scene.scene_id,
    ...
    Ok(BeautyV22GoldenCaptureReport {
        image_paths,
        timing_path,
        timings,
        scene_build_ms,
...
fn render_v22_golden_cell(
    ...
    for terrain in &cell.terrain {
        let base = material_color(
            ...
            seed ^ terrain.id,
            match cell.biome {
                WorldBiomeV22::NatureReserve => [55, 75, 46],
                WorldBiomeV22::Landfill => [88, 74, 54],
                WorldBiomeV22::City => [72, 69, 62],
            },
        );
    ...
    for curve in cell.curves.iter().take(32) {
        ...
        for pair in curve.points.windows(2) {
            ...
            image.world_line(
                cell.bounds,
                Vec3V22::new(pair[0].x, pair[0].y, pair[0].z + curve.sag_0_to_1 * 0.16),
                Vec3V22::new(pair[1].x, pair[1].y, pair[1].z + curve.sag_0_to_1 * 0.10),
    ...
    draw_texture_grain(image, seed ^ cell.cell_id ^ 0xD37A_2022);
}
...
fn draw_sky(
    ...
    for y in 0..sky_height {
        ...
        let upper = match biome {
            WorldBiomeV22::NatureReserve => [91.0, 122.0, 146.0],
            WorldBiomeV22::Landfill => [113.0, 122.0, 121.0],
            WorldBiomeV22::City => [100.0, 126.0, 149.0],
        };
...
fn draw_ground(
    ...
    let (surface_id, material_id, fallback) = match cell.biome {
        WorldBiomeV22::City => (
            BeautySurfaceIdV22(10_001),
            BeautyMaterialIdV22(1),
            [58, 58, 53],
        ),
        WorldBiomeV22::NatureReserve => (
            BeautySurfaceIdV22(20_001),
            BeautyMaterialIdV22(3),
            [48, 68, 41],
        ),
...
fn draw_facade(
    ...
    let (x1, y1_ground) = image.project(
        bounds,
        Vec3V22::new(facade.bounds.max.x, facade.bounds.max.y, 0.0),
    );
...
fn draw_vehicle(
    ...
    let body = material_color(
        ...
        seed,
        match vehicle.kind {
            VehicleKindV22::LandfillLoader => [142, 118, 58],
            _ => [47, 58, 64],
        },
    );
    let length = (vehicle.length_meters * 15.0).clamp(38.0, 92.0);
    let width = (vehicle.width_meters * 18.0).clamp(20.0, 52.0);
    image.fill_irregular_blob(
        [x, y],
    ...
    let wheel_r = match vehicle.kind {
        VehicleKindV22::LandfillLoader => vehicle.height_meters * 2.4,
        VehicleKindV22::Motorcycle => vehicle.height_meters * 1.7,
        _ => vehicle.height_meters * 1.9,
    }
    .clamp(3.0, 8.0);
    ...
    if vehicle.kind == VehicleKindV22::LandfillLoader {
        image.fill_polygon(
            &[
...
fn draw_human(
    ...
    let stride = match human.pose {
        HumanPoseStateV22::Walking => 5,
        HumanPoseStateV22::Running => 8,
        HumanPoseStateV22::Crouched | HumanPoseStateV22::Sitting => 2,
        HumanPoseStateV22::Idle => 3,
    };
...
fn write_timing_report(
    ...
    target_frame_ms: f32,
    scene_build_ms: f32,
    timings: &[BeautyV22CaptureTiming],
) -> io::Result<()> {
    let mut file = File::create(path)?;
    writeln!(file, "Ashfall V22 golden scene frame timings")?;
...
fn biome_label(biome: WorldBiomeV22) -> &'static str {
    match biome {
        WorldBiomeV22::City => "city",
        WorldBiomeV22::NatureReserve => "nature",
        WorldBiomeV22::Landfill => "landfill",
    }
}
...
fn mix_vec3(a: Vec3V22, b: Vec3V22, t: f32) -> Vec3V22 {
    Vec3V22::new(lerp(a.x, b.x, t), lerp(a.y, b.y, t), lerp(a.z, b.z, t))
}
...
impl RoadWetHighlight for RoadPatchV22 {
    fn wet_highlight_alpha(&self) -> f32 {
...
impl RgbImage {
    ...
    fn project(&self, bounds: Bounds3V22, point: Vec3V22) -> (i32, i32) {
    ...
    fn unproject_ground(&self, bounds: Bounds3V22, x: u32, y: u32) -> Vec3V22 {
        let margin_x = self.width as f32 * 0.07;
        let ground_top = self.height as f32 * GROUND_TOP_FRACTION;
        let ground_height = self.height as f32 - ground_top - self.height as f32 * 0.05;
...
mod tests {
    ...
    fn capture_human_painter_uses_body_masses_and_material_layers() {
        ...
        let bounds = Bounds3V22::new(Vec3V22::new(-1.0, -1.0, 0.0), Vec3V22::new(1.0, 1.0, 2.0));
        let mut human =
            HumanProxyV22::world_anchored_coherent_pedestrian(42, Vec3V22::new(0.0, 0.0, 0.0));
        human.pose = HumanPoseStateV22::Walking;
