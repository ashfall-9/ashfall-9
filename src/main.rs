...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args
        .iter()
        .any(|arg| arg == "--capture-v21-golden" || arg == "--capture-golden-v21")
    {
        let output_dir = output_dir_arg(&args)
            .unwrap_or_else(|| std::path::PathBuf::from("target/ashfall_v21_golden"));
        let report =
            ashfall::beauty_scene_v21_capture::capture_beauty_v21_golden_scenes(&output_dir)?;
        println!(
            "Captured {} V21 golden scene image(s) to {}",
